...
import { PostSaleData, PostSaleItemsData } from "../model/postData";
import LiveSearchProduct from "../pages/liveSearchProduct";
...
const CreateSale = ({ sales }) => {
  ...
  const handleProductPost = (e) => {
    e.preventDefault();
    PostSaleData(dataToPost);

    setTimeout(() => {
      PostSaleItemsData(saleItemsData);
      window.location.reload(true);
    }, 1000);
  };

  const handleOnChangeSaleItem = (e, index) => {
    if (dataToPost.OrderNumber === "")
      return alert("Please choose OrderNumber");

    const { name, value } = e.target;
    const list = [...saleItemsData];
    list[index][name] = value;
    list[index]["Sale"] = dataToPost.OrderNumber;
    setSaleItemsData(list);
  };
  ...
  const handleSaleItemRemove = (index) => {
    const list = [...saleItemsData];
    list.splice(index, 1);
    setSaleItemsData(list);
  };
            ...
            <div className="modal_input_div rounded-xl">
              <form className="space-y-6" action="#">
                <div className="grid grid-cols-2">
                  ...
                  <div className="bg-gray-200 w-[49rem] h-[12rem] overflow-auto rounded-xl">
                    <div className="">
                      <table className="">
                        <thead>
                          <tr>
                                ...
                                <td>
                                  {saleItemsData.length !== 1 && (
                                    <button
                                      ...
                                      onClick={() => handleSaleItemRemove(i)}
                                    >
                                      Delete
                                    </button>
                                  )}
